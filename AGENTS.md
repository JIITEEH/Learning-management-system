...
Node/Express on the server, MySQL for storage. The app sits at the
repository root.
...
- Add a resource by adding a router under `server/routes/` and mounting it
...
- A new permission code needs a row in `permissions` and a grant in
  `role_permissions` — add both to `database/seed.sql`.
...
- `database/schema.sql` is the single source of truth for structure.
